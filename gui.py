...
class CounterApp:
  ...
  def __init__(
    ...
    """Инициализирует GUI и загружает данные.

    Args:
      root: Корневое окно Tkinter.
      initial_readings: Список загруженных показаний.
  ...
  def _create_widgets(self) -> None:
    """Создаёт основные элементы управления интерфейса."""
    ...
    tk.Button(
        btn_frame, text="Обновить таблицу", command=self._refresh_table
    ).pack(side="left", padx=5)
    tk.Button(
        btn_frame, 
        text="[≡] Справка", 
  ...
  def _refresh_table(self) -> None:
    """Обновляет содержимое таблицы на основе списка показаний."""
  ...
  def add_reading(self) -> None:
    """Открывает диалоговое окно для добавления новой записи."""
    ...
    tk.Button(win, text="Сохранить", command=save).grid(row=3, column=1, pady=20, sticky="e")

  def delete_selected(self) -> None:
    """Удаляет выбранную в таблице запись из списка и файла."""
  ...
  def _show_help(self) -> None:
    """Имитирует HTML-страницу с двумя картинками-ссылками."""
    ...
    if self._img_doc:
      ...
      img_1.pack(pady=5)
      img_1.bind("<Button-1>", lambda e: webbrowser.open(url_1))
    ...
    if self._img_fold:
      ...
      img_2.pack(pady=5)
      img_2.bind("<Button-1>", lambda e: webbrowser.open(url_2))
